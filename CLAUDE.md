...
- **Phase 4 — the study + more data. ← current.** Three steps, in order:
  ...
  2. Build the **characterization table**: graph properties per dataset → the original-vs-augmented gap → a "when to augment" rule. ✅ **done** (`characterize.py --step all`, notebook 5). 11 keep original / 1 tie / 4 augment over 9 datasets. Three **candidate** LP rules clear the screen with 0 exceptions — augment when `components` < 39.5 (ρ = −0.86), `largest_component_frac` > 0.9588 (ρ = +0.79), `homophily_adjusted` < 0.2239 (ρ = −0.71) — the first two being one fragmentation variable read two ways. NC yields no rule: 0 of 7 usable cells augment, so there is nothing to separate. The professor-proposed homophily (NC) and density (LP) rules were **tested and not supported** (NC × adjusted homophily ρ = −0.36; LP × avg_degree ρ = +0.64, and sparse `roman_empire` augments) — report them as failed predictions, do not quietly swap in the post-hoc winners. Homophily and neighbour-label predictability are both reported class-balance-adjusted; raw values are not comparable across datasets with different class counts. Screen gates and their rationale live in `characterize.GATES`; full write-up in `docs/paper_log.md`.
  2b. **Module 3 — validation. 🔵 not started.** Freeze the candidate rules, pick unseen datasets, record the predicted verdict **before** training, then run the unchanged pipeline and score the prediction. Binding constraint: the held-out set must contain a **fragmented + heterophilous** graph and a **single-component + homophilous** graph, or the standing confound survives — the LP *keep original* cells are exactly the core-4 and the *augment* cells are exactly the four datasets added later, so `components` and dataset provenance are currently the same variable.
