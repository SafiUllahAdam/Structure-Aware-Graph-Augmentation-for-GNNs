...
- **Phase 4 — the study + more data. ← current.** Three steps, in order:
  ...
  **THE TWO FINAL CANDIDATE RULES (both link prediction, locked 2026-07-29):**
  1. **augment when `homophily_adjusted` < 0.227** — ρ −0.90 / **−1.00** on the bias-free gap, LODO min\|ρ\| 0.80, 0 exceptions, graded across 5 distinct values. Lead with this one.
  2. **augment when `largest_component_frac` > 0.9588** — ρ +0.78, LODO 0.71, 0 exceptions. Real but a **two-group split**: all four augment cells sit at exactly 1.0, so the cut is arbitrary in (0.9177, 1.0) and it coincides with the batch confound.

  `nbr_predictability_adjusted` < 0.4084 was **screened and dropped**: it clears on `gap_rel` only, is rejected by the fixed-variant control (ρ −0.60 < the 0.7 gate), and its LODO floor is 0.40 — one dataset moves it. Its LODO *sign* was stable; the failure is magnitude, not direction.
  ...
  **NC yields no rule at all** — 0 of 5 usable NC cells augment, so nothing can be separated; only the boundary "never augment" is reportable, and any held-out NC test is a falsification attempt.
  ...
  2b. **Module 3 — validation. 🔵 current.** The **two rules above are frozen**. Pick unseen datasets, record the predicted verdict **before** training, then run the unchanged pipeline and score the prediction. Binding constraint: the held-out set must contain a **fragmented + heterophilous** graph and a **single-component + homophilous** graph, or the standing confound survives — the LP *keep original* cells are exactly the two fragmented graphs and the *augment* cells are exactly the four fully-connected ones, so fragmentation and dataset provenance are currently the same variable. Rule 1 (homophily) and rule 2 (connectivity) disagree on such a graph, which is what makes it worth running.
