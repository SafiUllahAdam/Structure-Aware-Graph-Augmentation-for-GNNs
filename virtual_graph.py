...
class VirtualGraph():
    ...
    def signatures(self, sim):
        ...
        if sim == 'psi':
            ...
            raise ValueError(f"Unknown sim '{sim}'. Use: psi, degree, centrality.")
...
def main(args):
    ...
    if args.output is None:
        ...
        args.output = f"output/{ds}/k{args.k}/virtual_{args.sim}.edgelist"
    G = build_graph()
...
def parse_args():
    ...
    parser.add_argument('--output', nargs='?', default=None, help='Output virtual edgelist (default: output/<ds>/k<K>/virtual_<sim>.edgelist)')
    parser.add_argument('--sim', default='psi', choices=['psi', 'degree', 'centrality'],
                        help='Structural similarity: psi=I2V KL/Poisson, degree-only, centrality-only. Default psi.')
