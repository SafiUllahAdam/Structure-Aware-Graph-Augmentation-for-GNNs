...
| `results_io.py` | write `results/notebook1_reproduce_i2v/NNN.{date}.{dataset}.{task}.csv` with a JSON `#META` header + upsert `results/scoreboard.csv` |
...
python scripts/main.py --task linkpred --dataset cora --emb output/notebook1_reproduce_i2v/cora/node_classification/i2v_s42.emb

# Node classification (needs labels/cora.labels)
python scripts/main.py --task nodeclass --dataset cora --emb output/notebook1_reproduce_i2v/cora/node_classification/i2v_s42.emb
```
